...
- `lifeos isa validate /data/projects/IWantIt/ISA.md` now reports
  `openCriteria=0`, `hasVerification=true`, and `malformed=false`. The earlier
  noncanonical `## Verification Evidence` heading was the reason the
  completion hook could not discover the already-recorded evidence; the
  canonical section is now `## Verification`.
